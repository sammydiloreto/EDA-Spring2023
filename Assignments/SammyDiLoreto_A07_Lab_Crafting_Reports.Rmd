...
```{r 2018 boxplot, echo=FALSE, message=FALSE, warning=FALSE, fig.align= "left", fig.height= 3, fig.cap="Ozone Levels by Month in 2018"}
boxplot.2018 <- epa_data %>% 
  filter(Year == 2018) %>% 
  select(Month, Ozone) %>%
  drop_na() %>% 
  ...
  scale_x_discrete(
    drop=FALSE)+
  labs(x = "Month" ,
    y = expression(paste("Ozone", " (", mu, "g/m"^{3},")"))) #use expression paste, look in visual lab
print(boxplot.2018)

```

```{r 2019 boxplot, echo=FALSE, message=FALSE, warning=FALSE, fig.align= "left", fig.height= 3, fig.cap="Ozone Levels by Month in 2019"}
boxplot.2019 <- epa_data %>% 
  filter(Year == 2019) %>% 
  select(Month, Ozone) %>%
  drop_na() %>% 
  ggplot(mapping = aes(x= factor(Month, 
