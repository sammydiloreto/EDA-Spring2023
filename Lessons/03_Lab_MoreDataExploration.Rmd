...
vector1[3] # returns 5.3 which is 3rd item in vector1

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # counts up by columns
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #switching to go by row
matrix2
...
matrix3
matrix1[4, ] #gives all columns from fourth row
matrix1[ , 3] #gives all rows in column 3
matrix1[c(12, 14)] #12th and 14th value
matrix1[c(12:14)] #12th-14th value
matrix1[2:4, 1:3] #slices --> 2nd-4th row, 1st-3rd column
...
# Lists ---- semicolon is to run on one line
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3); list1
list1[[2]] #need to put double brackets to give item in list
...
> Answer: df and lists show up as data (more than one dimension), vectors and values go into values

Question: In the R chunk below, write "dataframe1\$". Press `tab` after you type the dollar sign. What happens?

> Answer: gives options of what to choose
...
```{r DataExercise-1a}
highTemp <- c(64,71,72,64,51,52,61,64,66,73)
lowTemp <- c(41,49,59,46,32,32,40,46,52,53)
```
...
f2c <- function(F) {
  C <- round(((F-32)*(5/9)),2)
  return(C)}
highTempC <- f2c(highTemp)
lowTempC <- f2c(lowTemp)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
temp.df <- data.frame(highTemp,lowTemp,highTempC, lowTempC)
names(temp.df) <- c("High Temps (F)","Low Temps (F)","High Temps (C)", "Low Temps (C)")
View(temp.df)
...
```{r DataExercise-1d}
summary(temp.df)
summary(temp.df$`Low Temps (F)`)
sd(temp.df$`High Temps (F)`) #only use on vector not whole df
...
format(today, format = "%Y") #y 2 digits year 
#format(today, format = "")
#format(today, format = "")
