...
[columns can be different types (number, characters), vectors or matrices need to be same type]
...
```{r}
cars$speed #just calling out as a vector but not a new dataframe
```
...
#Option 2
is.data.frame(car_speed) #just a vector
```

How could you make it a data frame?

```{r}
df_car_speed <- as.data.frame(car_speed) #transform any matrix or vector as a df
df_car_speed #assigns name of vector/matrix as column name in the df
class(df_car_speed) 
```

## Adding columns to a data frame

How could you add columns to *df_car_speed*? using the * * italicize the text, using ** ** bolds the text

```{r}
car_dist <- cars$dist

#Option 1 moving car_dist into df_car_speed --> cbind(df,vector), brings column from df_car_speed
...
```{r}
colnames(df)  #<- c() can use colnames to rename / or simply names()
names(df)
```
...
use ??function in console for help
```{r}
ncol(df)  
nrow(df)  
