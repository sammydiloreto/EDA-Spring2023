...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    ...
    #paste brings together strings, insert spaces between strings
    #expression is used to make mathematical formulas 
    )
...
surface_temp_PeterPaul_plot <- PeterPaul.chem.nutrients %>% 
  filter(depth == 0) %>% 
  ggplot(
    mapping = aes(
      x=daynum,
  ...
  geom_point(size=0.5, aplha=0.5)+ #alpha sets transparency 0 to 1
